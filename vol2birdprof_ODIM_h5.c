...
/*Adjusted to ODIM_H5 input format 15-07-2015*/
...
int read_ODIM_h5_scan(hid_t file,int iscan,char type,SCANMETA *meta,unsigned char *data[]);
...
// determine the number of scan groups 
Nscanin = 0;
sprintf(string, "/dataset%d", Nscanin + 1);
while (H5Lexists(h5_in, string, H5P_DEFAULT))
{
  Nscanin += 1;
  sprintf(string, "/dataset%d", Nscanin + 1);
}
...
// loop over the scan groups
for (m=0,n=1 ; n<=Nscanin ; n++) {
   if (!flag[n]) continue;
   result=read_ODIM_h5_scan(h5_in,n,'V',vmeta+m,vscan+m);
   if (result&&read_ODIM_h5_scan(h5_in,n,'Z',zmeta+m,zscan+m)){
      ...
      if (!read_ODIM_h5_scan(h5_in,n,'U',uzmeta+m,uzscan+m)) uzflag=0;
      if (cmflag == 1){
         if (read_ODIM_h5_scan(h5_cm,n,'M',cmmeta+m,cmscan+m)){
      ...
      if (dualpolflag == 1){
         if (!(read_ODIM_h5_scan(h5_in,n,'R',rmeta+m,rscan+m)&&read_ODIM_h5_scan(h5_in,n,'D',zdrmeta+m,zdrscan+m))){
...
printf("\n   Version 0.1.0 (15-Jul-2015)\n");
printf("\n   requires OPERA ODIM-h5 input format, see http://www.eumetnet.eu/opera-software\n");
...
/******************************************************************************/
/*This function reads radar scan data from an OPERA HDF5 file. The scan number*/
...
float x;
hid_t result;
/*Reading of metadata from top-level where group in file.*/
meta->heig=0;
H5LTget_attribute_float(file,"/where","height",&meta->heig);
meta->heig*=1e-3;
H5LTget_attribute_float(file,"/where","lat",&meta->lat);
H5LTget_attribute_float(file,"/where","lon",&meta->lon);

/*Reading of metadata from dataset (scan) group in file.*/
sprintf(object,"/dataset%d/what",iscan);
H5LTget_attribute_string(file,object,"startdate",string);
sscanf(string, "%d", &meta->date);
H5LTget_attribute_string(file,object,"starttime",string);
sscanf(string, "%d", &meta->time);

sprintf(object,"/dataset%d/where",iscan);
H5LTget_attribute_float(file,object,"elangle",&meta->elev);
H5LTget_attribute_int(file,object,"nbins",&meta->nrang);
H5LTget_attribute_int(file,object,"nrays",&meta->nazim);
H5LTget_attribute_float(file,object,"rscale",&meta->rscale);
meta->rscale /= 1000.0;
meta->ascale = 360.0 / ((float) meta->nazim);
H5LTget_attribute_int(file,object,"a1gate",&meta->azim0);

variable = 1;
found=0;
sprintf(object,"/dataset%d/data%d/what",iscan,variable);
while(H5Lexists(file, object, H5P_DEFAULT) == 1){
  H5LTget_attribute_string(file, object, "quantity", string);
  if ((type=='Z') && (strcmp(string, "DBZH") == 0))
  {
    H5LTget_attribute_float(file,object,"gain",&meta->zscale);
    H5LTget_attribute_float(file,object,"offset",&meta->zoffset);
    H5LTget_attribute_float(file,object,"undetect",&x);
    meta->missing = (int) x;
    found=1;
    break;
  }
  else if ((type=='U') && (strcmp(string, "TH") == 0))
  {
    H5LTget_attribute_float(file,object,"gain",&meta->zscale);
    H5LTget_attribute_float(file,object,"offset",&meta->zoffset);
    H5LTget_attribute_float(file,object,"undetect",&x);
  ...
  else if ((type=='V') && (strcmp(string, "VRAD") == 0))
  {
    H5LTget_attribute_float(file,object,"gain",&meta->zscale);
    H5LTget_attribute_float(file,object,"offset",&meta->zoffset);
    H5LTget_attribute_float(file,object,"undetect",&x);
  ...
  else if ((type=='W') && (strcmp(string, "WRAD") == 0))
  {
    H5LTget_attribute_float(file,object,"gain",&meta->zscale);
    H5LTget_attribute_float(file,object,"offset",&meta->zoffset);
    H5LTget_attribute_float(file,object,"undetect",&x);
  ...
  else if ((type=='R') && (strcmp(string, "RHO") == 0))
  {
    H5LTget_attribute_float(file,object,"gain",&meta->zscale);
    H5LTget_attribute_float(file,object,"offset",&meta->zoffset);
    H5LTget_attribute_float(file,object,"undetect",&x);
  ...
  else if ((type=='D') && (strcmp(string, "ZDR") == 0))
  {
    H5LTget_attribute_float(file,object,"gain",&meta->zscale);
    H5LTget_attribute_float(file,object,"offset",&meta->zoffset);
    H5LTget_attribute_float(file,object,"undetect",&x);
...
fprintf(stderr,"loading dataset%d-data%d ...",iscan,variable);

sprintf(object,"/dataset%d/data%d/data",iscan,variable);
result=H5LTread_dataset(file,object, H5T_NATIVE_UCHAR,*data);
/*Returning.*/

fprintf(stderr,"done, returning %d\n", !result);
return !result;
}
